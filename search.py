...
Description: This program will act as a search engine for articles, allowing users to find articles 
based on their desired criteria (keyword, title length, favorite, etc.)
'''
...
def favorite_article(favorite, titles):
    for article in titles:
        if article.lower() == favorite.lower():
            return True
            break
        else:
            continue
